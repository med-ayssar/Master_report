...
Finally, we have \emph{(built-in, built-in)} which means that both neuron and synapse model are already available in the NestKernel. As in the previous case, it is required that both the \emph{x\_\_with\_y} and \emph{y\_\_with\_x} are registered, or we will just stop the execution of the simulation script and throw an error telling the user that we cannot find the models. In all remaining cases, we just do nothing in the \texttt{ConnectWrapper} and let PyNEST's \texttt{Connect()} function handle them. \todo{if both synapse and neuron are NEST built-in, do we need to do anything at all (even mangle the names with the __with__ string inserted)? Would it go too far to mention deriving from the C++ ArchivingNode vs. StructuralPlasticityNode?}
